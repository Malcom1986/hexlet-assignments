...
class AppTest {
    ...
    void testIsBelongToOneQuadrant() {
        var segment1 = Segment.makeSegment(Point.makePoint(-3, -2), Point.makePoint(-1, -1));
        assertThat(App.isBelongToOneQuadrant(segment1)).isTrue();

        var segment2 = Segment.makeSegment(Point.makePoint(2, 2), Point.makePoint(10, 10));
        assertThat(App.isBelongToOneQuadrant(segment2)).isTrue();

        var segment3 = Segment.makeSegment(Point.makePoint(-2, 2), Point.makePoint(2, 1));
        assertThat(App.isBelongToOneQuadrant(segment3)).isFalse();

        var segment4 = Segment.makeSegment(Point.makePoint(0, 0), Point.makePoint(-2, -1));
        assertThat(App.isBelongToOneQuadrant(segment4)).isFalse();
    }
    // END
}
